...
public class IMMarginAnalystDataService {
    ...
    private IMMarginAnalystData mapIMMarginAnalystData(Row row, String plant, String strCurrency, Calendar monthYear, String orderNumber) {
        ...
        // if MarginAnalystMacro is existed -> get costRMB from this
        // if not -> the costRMB = dealerNet * 90% (exchange dealerNet in appropriate Currency if needed)
        ...
        if(matcher.find()) {
            ...
            for(Row row : sheet) {
                if(row.getRowNum() == 0) {
                    ...
                    if(!row.getCell(COLUMN_NAME.get("Order ID"), Row.MissingCellPolicy.CREATE_NULL_AS_BLANK).getStringCellValue().isEmpty())
                        orderNumber = row.getCell(COLUMN_NAME.get("Order ID"), Row.MissingCellPolicy.CREATE_NULL_AS_BLANK).getStringCellValue();
                    String strDate = row.getCell(COLUMN_NAME.get("Order Booked Date")).getStringCellValue();
                    if(!strDate.isEmpty()) {
                        monthYear = parseMonthYear(strDate);
                    }
                    IMMarginAnalystData imMarginAnalystData = mapIMMarginAnalystData(row, plant, strCurrency, monthYear, orderNumber);
    ...
    public void calculateMarginAnalystSummary(String fileUUID, String originalFileName, String durationUnit) {
        ...
        if(matcher.find()) {
            ...
            // Summarize the MarginAnalystData by OrderNumber(s)
            List<String> orderNumberList = getDistinctOrderNumber(fileUUID);
            for(String orderNumber : orderNumberList) {

                // forEach modelCode -> get MarginAnalystData -> then calculate MarginAnalystSummary
                List<String> modelCodeList = imMarginAnalystDataRepository.getModelCodesByFileUUID(fileUUID, orderNumber);
                for(String modelCode : modelCodeList) {
                    IMMarginAnalystSummary imMarginAnalystSummary = new IMMarginAnalystSummary();

                    List<IMMarginAnalystData> imMarginAnalystDataList = imMarginAnalystDataRepository.getIMMarginAnalystData(modelCode, strCurrency, fileUUID, orderNumber);
                    ...
                    log.info(modelCode + " - " + orderNumber + " - " + imMarginAnalystDataList.size());
                    ...
                    double manufacturingCostUSD = manufacturingCostRMB;
                    ...
                    if(plant.equals("HYM"))
                        manufacturingCostUSD = manufacturingCostRMB * aopRate;
                    ...
                    imMarginAnalystSummary.setManufacturingCost(manufacturingCostUSD);
    ...
    public List<IMMarginAnalystData> getIMMarginAnalystData(String modelCode, String strCurrency, String fileUUID, String orderNumber) {
        return imMarginAnalystDataRepository.getIMMarginAnalystData(modelCode, strCurrency, fileUUID, orderNumber);
    }
    ...
    public Map<String, Object> getIMMarginAnalystSummary(String modelCode, String strCurrency, String fileUUID, String orderNumber) {
        List<IMMarginAnalystSummary> imMarginAnalystSummaryList = imMarginAnalystSummaryRepository.getIMMarginAnalystSummary(modelCode, strCurrency, fileUUID, orderNumber);
    ...
    public List<String> getDistinctOrderNumber(String fileUUID) {
        return imMarginAnalystDataRepository.getOrderNumber(fileUUID);
    }
