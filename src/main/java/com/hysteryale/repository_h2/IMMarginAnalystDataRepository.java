...
public interface IMMarginAnalystDataRepository extends JpaRepository<IMMarginAnalystData, Integer> {
    ...
    @Query("SELECT m FROM IMMarginAnalystData m WHERE m.modelCode = ?1 AND m.monthYear = ?2 AND m.currency = ?3")
    List<IMMarginAnalystData> getEUPlantIMMarginAnalystData(String modelCode, Calendar monthYear, String currency);
